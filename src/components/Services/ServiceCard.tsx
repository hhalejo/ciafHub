...
interface ServiceCardProps {
  ...
  onContact: (receiverId: string) => void; // 👈 agregamos prop para contactar
}
...
export function ServiceCard({ service, onClick, onDelete, onContact }: ServiceCardProps) {
  ...
  // Nombre usuario seguro (soporta user.nombre o user.full_name si existen)
  const userName =
    // @ts-ignore
  ...
  return (
    ...
    >
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          onContact(service.user_id); // 👈 dueño del servicio
        }}
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-80">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">¿Eliminar servicio?</h2>
      ...
      <div className="flex justify-between items-start mb-3">
        <h3 className="font-semibold text-lg text-gray-900 line-clamp-2">{service.title}</h3>
